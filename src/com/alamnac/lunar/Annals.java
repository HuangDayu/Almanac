...
public class Annals {
	...
	public static final String[] NUMBER_CN = { "零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十" }; // 中文数字
	...
	public static final String[] XINGZUO = { "摩羯", "水瓶", "双鱼", "白羊", "金牛", "双子", "巨蟹", "狮子", "处女", "天秤", "天蝎", "射手" };
	public static final String[] YUEXIANG = { "朔月", "上弦", "望月", "下弦" }; // 月相名称表
	public static final String[] JIEQI = { "冬至", "小寒", "大寒", "立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种",
			"夏至", "小暑", "大暑", "立秋", "处暑", "白露", "秋分", "寒露", "霜降", "立冬", "小雪", "大雪" };
	//public static final String[] MONTH_NAME = { "十一", "十二", "正", "二", "三", "四", "五", "六", "七", "八", "九", "十" }; // 月名称,建寅
	public final static String[] MONTH_NAME_BIG_CN = {   "十一", "十二", "正", "贰", "弎", "肆", "伍", "陸", "柒", "捌", "玖", "什"};
	public static final String[] DAY_NAME = { "初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二",
	...
	static {
		...
		String s = "-2069,45,0,夏,禹,,禹,-2024,10,0,夏,启,,启,-2014,25,0,夏,太康,,太康,-1986,14,0,夏,仲康,,仲康,-1972,28,0,夏,相,,相,-1944,2,0,夏,后羿,,后羿,-1942,38,0,夏,寒浞,,寒浞,-1904,21,0,夏,少康,,少康,-1883,17,0,夏,杼,,杼,-1866,26,0,夏,槐,,槐,-1840,18,0,夏,芒,,芒,-1822,16,0,夏,泄,,泄,-1806,59,0,夏,不降,,不降,-1747,21,0,夏,扃,,扃,-1726,21,0,夏,廑,,廑,"
				...
				+ "1862,13,0,清,穆宗,爱新觉罗载淳,同治,1875,34,0,清,德宗,爱新觉罗载湉,光绪,1909,3,0,清,无朝,爱新觉罗溥仪,宣统,1912,37,0,近、现代,中华民国,,民国,1949,9999,1948,当代,新中国,,公历纪元";
	...
	public static void getDayName(LunarDate date, LunarDate dateTwo) {
		...
		int holiday = dateTwo.getHoliday_INT();

		// 按农历日期查找重量点节假日
		String d = date.getLunar_MonthName() + (date.getLunar_MonthName().length() < 2 ? "月" : "")
				+ date.getLunar_DayName();
		if (!date.getLunar_Lunar_isLeap().equals("闰")) {
			if (d.equals("正月初一")) {
				impHappyName += "春节 ";
				holiday = 1;
			}
		...
		if (date.getLunar_NextLunarMonthName().equals("正")) { // 最后一月
			if (d.equals("十二三十") && date.getLunar_Days_OfMonth() == 30) {
				impHappyName += "除夕 ";
				holiday = 1;
			}
			if (d.equals("十二廿九") && date.getLunar_Days_OfMonth() == 29) {
				impHappyName += "除夕 ";
				holiday = 1;
			}
			if (d.equals("十二廿三")) {
		...
		if (date.getLunar_SolarTerm() != null && !date.getLunar_SolarTerm().equals("")) {
			if (date.getLunar_SolarTerm().equals("清明")) {
				impHappyName += date.getLunar_SolarTerm() + " ";
				holiday = 1;
			} else {
				impName += date.getLunar_SolarTerm() + " ";
			}
		}

		// 农历杂节
		String w, w2;
		if (date.getLunar_Last_Dongzhi() >= 0 && date.getLunar_Last_Dongzhi() < 81) { // 数九
			w = NUMBER_CN[(int) (Math.floor(date.getLunar_Last_Dongzhi() / 9) + 1)];
			if (date.getLunar_Last_Dongzhi() % 9 == 0) {
				impName += "『" + w + "九』 ";
			} else {
				allName += w + "九第" + (date.getLunar_Last_Dongzhi() % 9 + 1) + "天 ";
			}
		}

		w = Common.subString(date.getChineseEra_Day(), 0, 1);
		w2 = Common.subString(date.getChineseEra_Day(), 1, 2);
		if (date.getLunar_Last_Xiazhi() >= 20 && date.getLunar_Last_Xiazhi() < 30 && w.equals("庚")) {
			impName += "初伏 ";
		}
		if (date.getLunar_Last_Xiazhi() >= 30 && date.getLunar_Last_Xiazhi() < 40 && w.equals("庚")) {
			impName += "中伏 ";
		}
		if (date.getLunar_Last_Liqiu() >= 0 && date.getLunar_Last_Liqiu() < 10 && w.equals("庚")) {
			impName += "末伏 ";
		}
		if (date.getLunar_Last_Mangzhong() >= 0 && date.getLunar_Last_Mangzhong() < 10 && w.equals("丙")) {
			impName += "入梅 ";
		}
		if (date.getLunar_Last_Xiaoshu() >= 0 && date.getLunar_Last_Xiaoshu() < 12 && w2.equals("未")) {
			impName += "出梅 ";
		}
		...
		dateTwo.setHoliday_INT(holiday);
	}
	...
	public static double qi_accurate(double W) {
		double t = AstronomyArithmetic.S_aLon_t(W) * 36525;
	...
	public static double so_accurate(double W) {
		double t = AstronomyArithmetic.MS_aLon_t(W) * 36525;
		return t - Common.dt_T(t) + (double) 8 / 24;
	}
