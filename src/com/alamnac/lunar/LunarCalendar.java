...
public class LunarCalendar {
	...
	private void lunarCalendarCalc(int year, int month, int hours) {
		...
		lunarDateCalc1.setLunarDate_China_Era_Year(Annals.TIANGAN[ChineseEra_Year % 10] + Annals.DIZHI[ChineseEra_Year % 12]); // 干支纪年
		...
		for (int i = 0, j = 0; i < julianMutualValue_month; i++) {
			...
			if (lunarDateCalc.getJulian_Days() == qiShuo.HS[mk] || lunarDateCalc.getJulian_Days() == julianMutualValue_year) { // 月的信息
	...
	public String getNowSolarTerm() {
		...
		for (String str : strs) {
			...
			if(now <= solarterm) {
				str = str.substring(str.length()-2,str.length())+" "+strs_3[1]+"月"+strs_3[2]+"日"+str.substring(10,str.length()-2);
