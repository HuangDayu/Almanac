...
public class AnnalsUtils {
	...
	public static void getDayName(AlmanacDTO date, AlmanacDTO dateTwo) {
		...
		if (date.getLunarNextLunarMonthName().equals("正")) { // 最后一月
			if (d.equals("十二三十") && date.getLunarDaysOfMonth() == 30) {
				impHappyName += "除夕 ";
				holiday = 1;
			}
			if (d.equals("十二廿九") && date.getLunarDaysOfMonth() == 29) {
				impHappyName += "除夕 ";
				holiday = 1;
			}
			if (d.equals("十二廿三")) {
		...
		// 农历杂节
		String w, w2;
		if (date.getLunarLastDZ() >= 0 && date.getLunarLastDZ() < 81) { // 数九
			w = NUMBER_CN[(int) (Math.floor(date.getLunarLastDZ() / 9) + 1)];
			if (date.getLunarLastDZ() % 9 == 0) {
				impName += "『" + w + "九』 ";
			} else {
				allName += w + "九第" + (date.getLunarLastDZ() % 9 + 1) + "天 ";
			}
		}

		w = CommonUtils.subString(date.getChineseEraDay(), 0, 1);
		w2 = CommonUtils.subString(date.getChineseEraDay(), 1, 2);
		if (date.getLunarLastXZ() >= 20 && date.getLunarLastXZ() < 30 && w.equals("庚")) {
			impName += "初伏 ";
		}
		if (date.getLunarLastXZ() >= 30 && date.getLunarLastXZ() < 40 && w.equals("庚")) {
			impName += "中伏 ";
		}
		if (date.getLunarLastLQ() >= 0 && date.getLunarLastLQ() < 10 && w.equals("庚")) {
			impName += "末伏 ";
		}
		if (date.getLunarLastMZ() >= 0 && date.getLunarLastMZ() < 10 && w.equals("丙")) {
			impName += "入梅 ";
		}
		if (date.getLunarLastXS() >= 0 && date.getLunarLastXS() < 12 && w2.equals("未")) {
			impName += "出梅 ";
		}
		...
		dateTwo.setHolidayDayIndex(holiday);
	}
