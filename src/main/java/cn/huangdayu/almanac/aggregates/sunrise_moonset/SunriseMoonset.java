...
public class SunriseMoonset extends AbstractAlmanac {
    ...
    private static final double RAD = 180.0 * 3600 / Math.PI;

    public SunriseMoonset(TimeZoneDTO timeZoneDTO) {
        sunTime(timeZoneDTO);
        moonTime(timeZoneDTO);
    }
     ...
     * @param timeZoneDTO
     */
    private void sunTime(TimeZoneDTO timeZoneDTO) {
        double midDayTimeValue = 0, dawnTimeValue = 0;
        ...
        for (int i = 0; i < 10; i++) {
            double[] doubles = sunRiseTime(richu, jd, wd, timeZoneDTO.getTimeZoneIndex() / 24);// 逐步逼近法算10次
            richu = doubles[0];
            midDayTimeValue = doubles[1];
            dawnTimeValue = doubles[2];
        }

        // 日出
        this.setSunRiseTime(doubleToStr(richu));
        // 日落
        this.setSunSetTime(doubleToStr(midDayTimeValue + midDayTimeValue - richu));
        // 中天
        this.setMidDayTime(doubleToStr(midDayTimeValue));
        // 天亮
        this.setDawnTime(doubleToStr(dawnTimeValue));
        // 天黑
        this.setDarkTime(doubleToStr(midDayTimeValue + midDayTimeValue - dawnTimeValue));
        // 昼长 = 日落-日出
        this.setDiurnalTime(doubleToStr((midDayTimeValue - richu) * 2 - 0.5));
        // 夜长
        this.setNightTime(doubleToStr(24 - ((midDayTimeValue - richu) * 2 - 0.5)));
     ...
     * @return 太阳升起时间
     */
    private double[] sunRiseTime(double date, double lo, double la, double tz) {
        double midDayTimeValue = 0, dawnTimeValue = 0, sunRiseTime = 0;
        ...
        // 严格应当区分极昼极夜，本程序不算
        if (cosH0 >= 1 || cosH0 <= -1) {
            return new double[]{-0.5, 0, 0};// 极昼
        }
        double H0 = -Math.acos(cosH0); // 升点时角（日出）若去掉负号 就是降点时角，也可以利用中天和升点计算
        double H1 = -Math.acos(cosH1);
        double H = gst - lo - a; // 太阳时角
        midDayTimeValue = date - degree(H) / Math.PI / 2 + tz; // 中天时间
        dawnTimeValue = date - degree(H - H1) / Math.PI / 2 + tz;// 天亮时间
        sunRiseTime = date - degree(H - H0) / Math.PI / 2 + tz; // 日出时间，函数返回值
        return new double[]{sunRiseTime, midDayTimeValue, dawnTimeValue};
    }
    ...
    private void moonTime(TimeZoneDTO timeZoneDTO) {
        ...
        find_moonrise_set(mjdd, timeZoneDTO.getTimeZoneIndex(), dbLon, dbLat, 0, 0);
    }

    private void find_moonrise_set(double mjd, double tz, double glong, double glat, int dls, int ST) {
        double moonRiseValue = 0, moonSetValue = 0;
        ...
        if (rise == true || sett == true) {
            if (rise == true) {
                if (ST == 0) {
                    moonRiseValue = utrise + dls;
                } else {
                    moonRiseValue = lmst(mjd + (utrise - tz) / 24, glong);
                }
            } else {
                moonRiseValue = -1;
            }
            if (sett == true) {
                if (ST == 0) {
                    moonSetValue = utset + dls;
                } else {
                    moonSetValue = lmst(mjd + (utset - tz) / 24, glong);
                }
            } else {
                moonSetValue = -1;
            }
         ...
         * @return
         */
        this.setMoonRiseTime(timeToStr(moonRiseValue));
        /***
         * 月落时间
         *
         * @return
         */
        this.setMoonSetTime(timeToStr(moonSetValue));
        /***
         * 月中时间
         *
         * @return
         */
        this.setMoonMiddleTime(timeToStr((moonSetValue + moonRiseValue) / 2));
