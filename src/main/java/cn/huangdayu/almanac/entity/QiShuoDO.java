    ...
    public static int calc(double jd, String qs) {
        ...
        if (jd >= f2 && jd < f3) { // 定气或定朔
            if (qs.equals("气")) {
                ...
                n = CommonUtils.subString(QB, index, index + 1); // 找定气修正值
            } else {
                ...
                n = CommonUtils.subString(SB, index, index + 1); // 找定朔修正值
            }
            if (n.equals("1")) {
                return (int) (D + 1);
            }
    ...
    public void calcY(int jd) {
        ...
        if (YY >= -721 && YY <= -104) {
            ...
            for (i = 0; i < 3; i++) {
                ...
                // 颁行历年首, 闰月名称, 月建
                if (yy >= -721) {
                    ns[i] = calc(1457698 - CommonUtils.JULIAN_FOR_2000 + Math.floor(0.342 + (yy + 721) * 12.368422) * 29.5306,
                            "朔");
                    ns[i + 3] = "十三";
                    ns[i + 6] = 2;
                } // 春秋历,ly为-722.12.17
                if (yy >= -479) {
                    ns[i] = calc(1546083 - CommonUtils.JULIAN_FOR_2000 + Math.floor(0.500 + (yy + 479) * 12.368422) * 29.5306,
                            "朔");
                    ns[i + 3] = "十三";
                    ns[i + 6] = 2;
                } // 战国历,ly为-480.12.11
                if (yy >= -220) {
                    ns[i] = calc(1640641 - CommonUtils.JULIAN_FOR_2000 + Math.floor(0.866 + (yy + 220) * 12.369000) * 29.5306,
                            "朔");
                    ns[i + 3] = "后九";
                    ns[i + 6] = 11;
                } // 秦汉历,ly为-221.10.31
            ...
            for (i = 0; i < 14; i++) {
                ...
                if (f1 < 12) {
                    ym[i] = AnnalsUtils.MONTH_NAME_BIG_CN[(f1 + Integer.parseInt(ns[nn + 6].toString())) % 12];
                } else {
        ...
        // 名称转换(月建别名)
        for (i = 0; i < 14; i++) {
            int Dm = HS[i] + CommonUtils.JULIAN_FOR_2000, v2 = yIndex[i]; // Dm初一的儒略日,v2为月建序号
            String mc = AnnalsUtils.MONTH_NAME_BIG_CN[v2 % 12]; // 月建对应的默认月名称：建子十一,建丑十二,建寅为正……
            if (Dm >= 1724360 && Dm <= 1729794) {
                mc = AnnalsUtils.MONTH_NAME_BIG_CN[(v2 + 1) % 12]; // 8.01.15至 23.12.02 建子为十二,其它顺推
            } else if (Dm >= 1807724 && Dm <= 1808699) {
                mc = AnnalsUtils.MONTH_NAME_BIG_CN[(v2 + 1) % 12]; // 237.04.12至239.12.13 建子为十二,其它顺推
            } else if (Dm >= 1999349 && Dm <= 1999467) {
                mc = AnnalsUtils.MONTH_NAME_BIG_CN[(v2 + 2) % 12]; // 761.12.02至762.03.30 建子为正月,其它顺推
