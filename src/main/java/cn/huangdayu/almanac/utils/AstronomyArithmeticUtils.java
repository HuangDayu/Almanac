...
public class AstronomyArithmeticUtils {
	...
	public static double gxc_sunLon(double t) { // 太阳光行差,t是世纪数
		...
		return (-20.49552 * (1 + e * Math.cos(v))) / CommonUtils.RAD; // 黄经光行差
	}
	...
	public static double S_aLon(double t, int n) {
		return E_Lon(t, n) + CommonUtils.nutationLon2(t) + gxc_sunLon(t) + Math.PI; // 注意，这里的章动计算很耗时
	}
	...
	public static double S_aLon_t2(double W) {
		...
		t += (W - E_Lon(t, 8) - Math.PI + (20.5 + 17.2 * Math.sin(2.1824 - 33.75705 * t)) / CommonUtils.RAD) / v;
		return t;
	}
